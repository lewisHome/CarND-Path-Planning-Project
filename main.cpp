...
int main() {
  ...
  int lane = 1;
  float lane_pos =1.0;
  bool lane_change_direction = true; //if true car is moving towards lane 2 if false car is moving towards lane 0
  ...
  h.onMessage([&map_waypoints_x,&map_waypoints_y,&map_waypoints_s,&map_waypoints_dx,&map_waypoints_dy,&ref_vel,&lane, &lane_pos, &lane_change_direction, &lane_change_complete, &max_s](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length,
                     uWS::OpCode opCode) {
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
		...
		//this is a sparese list of way points
		vector<double> ptsx;
		vector<double> ptsy;
		...
		//find surrounding vehicles forward
		double lane_0_next_car_s=100000;
		double lane_1_next_car_s=100000;
		double lane_2_next_car_s=100000;
		
		//find surrounding vehicles rear
		double lane_0_rear_car_s=-100000;
		double lane_1_rear_car_s=-100000;
		...
		for(int i=0; i< sensor_fusion.size(); i++)
		{
			...
			else if (next_car_d >= 4 && next_car_d <= 8)
			{
				next_car_lane = 1;
			}

			else if (next_car_d > 8 && next_car_d <=12)
			{
				next_car_lane = 2;
			}
			else
			...
			//determine how close adjacent cars are in each lane
...
next_iteration:;	
		...
		else if(current_lane == 1 && lane_1_next_car_s-car_s < lane_match_speed_dist) 
		{
			current_lane_blocked = true;
			lane_speed=lane_1_speed;
		}
		else if(current_lane == 2 && lane_2_next_car_s-car_s < lane_match_speed_dist)
		{
			current_lane_blocked = true;
			lane_speed=lane_2_speed;
		}
		else
		{
		...
		{	
			...
			else if(current_lane == 1 && lane_2_open==1 && lane_2_next_car_s > lane_0_next_car_s) 
			{
				lane=2;
				lane_change_complete = false;
				lane_change_direction = true;
		...
		vector<double> next_wp2 = getXY(car_s+90,(2+4*lane_pos),map_waypoints_s,map_waypoints_x,map_waypoints_y);
		...
		{
	//		cout<<"Refereance Velocity"<<ref_vel<<endl;
			if(ref_vel > lane_speed)
			{
				ref_vel -= 0.224;
			}
			else if(ref_vel < lane_speed)
			{
				ref_vel += 0.224;
			}
